...
function apply(w,a,combat=false){
 for(let i=0;i<24;i++){
  ...
  const shoot=combat?!b?.exhausted:threat;
...
function plan(start,target,combat=false){
 let beam=[{w:clone(start),route:[],cost:0}];
 for(let depth=0;depth<10;depth++){
  const next=[],seen=new Set();
  for(const node of beam)for(let id=0;id<actions.length;id++){
   const a=actions[id],w=clone(node.w);apply(w,a,combat);if(w.stats.deaths>start.stats.deaths)continue;
