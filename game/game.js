(function(){
  ...
  let upgradeChoice=null,renderPrevious=null;
  const DEATH_THEMES={
  ...
  function showUpgrade(after){
    ...
    dialog('НОВАЯ ЛЕММА · ВЫБОР ДО КОНЦА ИГРЫ','Как продолжим погоню?',`<p>${world.index===0?'Иван скрылся за дверью. Есть минута доработать метод.':'Перед продолжением выберите новое улучшение для этого сохранения.'}</p><div class="upgrade-grid"><article><canvas id="homing-preview" width="380" height="145" aria-label="Челюсть с ракетным соплом"></canvas><h3>Зубная баллистика</h3><p>Челюсть сама поворачивает к ближайшему противнику. Сопло и пламя показывают направление полёта.</p><small>J / X · работает при каждом выстреле<br>Щиты и земляные стенки остаются препятствиями.</small></article><article><canvas id="paper-preview" width="380" height="145" aria-label="Гуров ест бумагу"></canvas><h3>Питательная рукопись</h3><p>Нажмите H, чтобы съесть лист и восстановить 2 сердца за 1,2 секунды.</p><small>20 секунд с начала попытки · бумага бесконечна<br>Нужно стоять на земле. Удар, прыжок или рывок прервёт еду.</small></article></div><p class="upgrade-note">Выберите одно улучшение. Его нельзя заменить до новой игры.</p>`,[{text:'Выбрать самонаведение →',action:()=>choose('homing')},{text:'Выбрать лечение · H →',action:()=>choose('paper')}]);
  ...
  const handled=['ArrowLeft','ArrowRight','ArrowUp','Space','KeyA','KeyD','KeyW','KeyJ','KeyX','KeyK','KeyH','ShiftLeft','ShiftRight','KeyE','Enter','Escape','KeyP','KeyM'];
  ...
  const resumeMenuAudio=()=>{if((mode==='menu'||mode==='credits'||mode==='boss-outro'||mode==='dying'||mode==='epilogue'||mode==='prologue'||mode==='modal'||story)&&!document.hidden&&sound.enabled)sound.init();};
  addEventListener('focus',()=>{resumeMenuAudio();if(story&&!sound.voiceJob){const frame=GurovStory[story.kind][story.index];sound.speak(frame.actor,frame.text,4);}if(epilogue?.phase==='line'&&!sound.voiceJob)epilogue.spoken=false;});
  addEventListener('blur',()=>{keys.clear();pressed.clear();released.clear();if(mode==='play')pause();sound.suspend(!!prologue);});
  document.addEventListener('visibilitychange',()=>{if(document.hidden){keys.clear();if(mode==='play')pause();sound.suspend(!!prologue);}else{resumeMenuAudio();if(epilogue?.phase==='line'&&!sound.voiceJob)epilogue.spoken=false;}});
  addEventListener('beforeunload',()=>{persist();sound.close();});
  function input(){return {heal:pressed.has('KeyH'),interact:pressed.has('KeyE'),skipIntro:pressed.has('Enter')||pressed.has('Space'),left:keys.has('KeyA')||keys.has('ArrowLeft'),right:keys.has('KeyD')||keys.has('ArrowRight'),jump:['Space','KeyW','ArrowUp'].some(k=>pressed.has(k)),jumpReleased:['Space','KeyW','ArrowUp'].some(k=>released.has(k)),shoot:keys.has('KeyJ')||keys.has('KeyX'),dash:['ShiftLeft','ShiftRight','KeyK'].some(k=>pressed.has(k))};}
  ...
  function events(){for(const e of world.events){
    ...
    if(e.type==='paperInterrupted')toast('Приём пищи прерван. Найдите момент поспокойнее.',2);
  ...
  function updateHud(){
    ...
    set('upgrade-status',world.upgrade==='homing'?'J · САМОНАВОДЯЩАЯСЯ ЧЕЛЮСТЬ':p.eating>0?'H · ЖУЁМ БУМАГУ…':world.paperCooldown>0?`H · БУМАГА ЧЕРЕЗ ${Math.ceil(world.paperCooldown)} С`:p.hp>=5?'H · БУМАГА · ЗДОРОВЬЕ ПОЛНОЕ':'H · СЪЕСТЬ БУМАГУ · +2 ♥');
  ...
  function loop(t){
    const dt=Math.min((t-last)/1000||0,.05);last=t;clock+=dt;shake=Math.max(0,shake-dt);sound.update();
